...
# 4. Create summary_table: average Conversion Rate by month for each source/mediu
...
# 5. Print the average Conversion Rate by month
print(summary_table)
...
# 7. Build and print a table of the Top 5
top5_table <- period_summary %>%
  group_by(Source_Medium) %>%
  summarise(
    Total_Sessions = sum(Total_Sessions, na.rm = TRUE),
    .groups = "drop"
  ) %>%
