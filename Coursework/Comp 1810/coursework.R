...
# Identify top 3 sources overall
top_sources <- df %>%
...
# Filter for top 3 and prepare for plotting
top3_revenue <- df %>%
...
#Task B (use Source/Medium)
# 1. Import required libraries
library(tidyverse)
library(scales)

# 2. Load raw data
data_path <- "E:/Coursework/Comp 1810/Data for CW-20250626/Data for CW/Web Analytic_Dataset.csv"
...
# 4. Create summary_table: total sessions & average conversion rate by month & source
...
# 6. Detailed bar‐chart of average Conversion Rate by Source/Medium
...
top5_table <- period_summary %>%
  group_by(Source_Medium) %>%
  summarise(
    Period_Avg_Conv = round(mean(Avg_Conversion_Rate, na.rm = TRUE), 2),
    .groups = "drop"
  ) %>%
...
# 5. Prepare relationship_df for the four custom scatter plots
relationship_df <- df %>%
  rename(
...
# 1. Read the data into df2
df2 <- read_csv("E:/Coursework/Comp 1810/Data for CW-20250626/Data for CW/diabetes.csv")
...
# 9. Draw histograms titled “Frequency chart of … after cleaning”
for (col_name in names(df2_clean)) {
  if (is.numeric(df2_clean[[col_name]])) {
    p <- ggplot(df2_clean, aes_string(x = col_name)) +
      geom_histogram(bins = 30, fill = "pink", color = "black", alpha = 0.7) +
      labs(
        title = paste("Frequency chart of", col_name, "after cleaning"),
        x     = col_name,
        y     = "Frequency"
      ) +
      theme_light()
