...
#Task B (use Source/Medium as Devices)
...
# Read the dataset and Clean,rename.
...
# Summarize users by device and print summary table.
user_summary <- df %>%
  group_by(Device) %>%
  summarise(
...
for (col_name in names(df2_clean)) {
  if (col_name == "Outcome") {
    # Pie chart for Outcome counts
    outcome_df <- df2_clean %>%
      count(Outcome) %>%
      mutate(label = paste0(Outcome, " (", n, ")"))
    
    # Base R pie chart
    pie(
      outcome_df$n,
      labels = outcome_df$label,
